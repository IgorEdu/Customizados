#include 'protheus.ch'
#define __codBanco "748"                                                                                                                      
#define __nomBanco "COOPERATIVA DE CRÉDITO, POUPANÇA E INVESTIMENTO CAMPOS GERAIS – SICREDI CAMPOS GERAIS PR/SP"
#define nLarg  (605-35)
#define nAlt   842
/***********************************************************************************
	Fonte para emissão do Boleto do banco Sicredi
*************************************************************************************/
User Function RBol748(oBoleto)
	Local nBloco1 := 0
	Local nBloco2 := 0
	Local nBloco3 := 0
	Local nPont
	...
	//nosso numero
	Private cNossoNumero := U_RBOL748A(SEE->EE_TPCOBRA) //mODALIDADE COBRANÇA 
	...
	//Nome do Banco
	oBoleto:Say(nBloco1+33,25,"SICREDI",oArial12N )
	//logo
	oBoleto:SayBitmap(nBloco1+20, 20, "\system\logo-banco-748.jpg", 75, 20)
	...
	//Numero do Banco
	oBoleto:Say(nBloco1+35,99,"748-X", oArial18N )
	...
	//Nome do Banco
	oBoleto:Say(nBloco2+170,25,"SICREDI",oArial12N )
	//logo
	oBoleto:SayBitmap(nBloco2+157, 20, "\system\logo-banco-748.jpg", 75, 20)
	//Line(linha_inicial, coluna_inicial, linha final, coluna final)
	...
	//Numero do Banco
	oBoleto:Say(nBloco2+174,99,"748-X",oArial18N )
...
//	oBoleto:FWMSBAR("INT25" ,61,2.3, cCodigoBarra ,oBoleto,.F.,,.T.,0.02,1,.F.,"Arial",NIL,.F.,2,2,.F.)
	...
	//Nome do Banco
	oBoleto:Say(nBloco3+413,25,"SICREDI",oArial12N )
	//logo
	oBoleto:SayBitmap(nBloco3+400, 20, "\system\logo-banco-748.jpg", 75, 20)
	//Line(linha_inicial, coluna_inicial, linha final, coluna final)
	...
	//Numero do Banco
	oBoleto:Say(nBloco3+417,99,"748-X",oArial18N )
	//linha digitavel
...
Static Function ImprimeBloco(oBoleto, nBloco)
	...
	oBoleto:Say(nBloco+197,25 ,"PAGÁVEL PREFERENCIALMENTE NAS COOP.DE CRÉDITO DO SICREDI",oArial09N)
	...
	oBoleto:SayAlign(nBloco+212,435,AllTrim(SEE->EE_AGENCIA)+".09."+Alltrim(SEE->EE_CODEMP) ,oArial09N,100,10,,1)
	...
	oBoleto:Say(nBloco+243,237, "DMI"										,oArial09N) //Tipo do Titulo
	...
	oBoleto:Say(nBloco+233,425 ,"Nosso Número",oArial06)
	oBoleto:SayAlign(nBloco+234,435,Transform(cNossoNumero,"@R 99/999999-9"),oArial09N,100,10,,1)
	...
	IF SA1->A1_PESSOA == "J"
		oBoleto:Say(nBloco+376,400 ," - CNPJ: " + transform(SA1->A1_CGC,"@R 99.999.999/9999-99") ,oArial09N)
	Else
		oBoleto:Say(nBloco+376,400 ," - CPF: " + transform(SA1->A1_CGC,"@R 999.999.999-99") ,oArial09N)
	EndIF
	oBoleto:Say(nBloco+406,450 ,Transform(cNossoNumero,"@R 99/999999-9") ,oArial09N)
	...
	oBoleto:Say(nBloco+406, 25, "Sacador/Avalista" , oArial06)
...
/*    
    ...
    oBoleto:Say(nBloco+432, 25, "                    Ouvidoria: 0800 725 7474.                           ",oArial09N)
    oBoleto:Say(nBloco+439, 25, "                         caixa.gov.br                                   ",oArial09N)
*/	oBoleto:FWMSBAR("INT25" ,61,2.3, cCodigoBarra ,oBoleto,.F.,,.T.,0.02,1.5,.F.,"Arial",NIL,.F.,2,2,.F.)

Return
/*******************************************************************/
User Function RBOL748A(_TpCart)
Local _cNum  := STRZERO(VAL(SE1->E1_NUMBCO),5)
Local _cDig
Local _cAno := substr(DTOS(SE1->E1_EMISSAO),3,2)
Local _cEmisBloq := "4" //4-Cedente //Particularidade CEF
Default _TpCart := "1"	

_cDig := BolDigitoBarra(_cAno + "2" +_cNum,"NOSSONUM")

Return _cAno+"2"+_cNum+_cDig
/******************************************************************
...
Static Function BolCodBar()
	...
	//Pos 20 - Tipo de Cobrança //1 - Cobrança registrada
	cCodigo += "1"
	//Pos 21 - Tipo de Carteira //1 - Carteira Simples
	cCodigo += "1"
	//Pos 22 a 30 - Nosso numero
	cCodigo += cNossoNumero
	//Pos 31 a 34 - Cooperativa de crédito/agência beneficiária
	cCodigo += AllTrim(SEE->EE_AGENCIA)
	//Pos 35 a 36 -  Posto da cooperativa de crédito/agência beneficiária
	cCodigo += "09"
	//Pos 37 a 41 -  Código do beneficiário
	cCodigo += Alltrim(SEE->EE_CODEMP)
	//Pos 42 - “1” (um) quando houver valor expresso no campo “valor do documento”
	//“0” (zero) quando valor expresso no campo “valor do documento” for zerado 
	cCodigo += "1"
	//Pos 43 - Filler – zeros 
	cCodigo += "0"
	//Pos 44 - DV do campo livre calculado por módulo 11 com aproveitamento total
