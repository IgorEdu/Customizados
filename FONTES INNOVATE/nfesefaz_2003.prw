...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						//Douglas Negrello -  customizacao para mensagens necessarias na NF-e
						If Empty(cMensFis) .AND. SF2->(FieldPos("F2_XMENCOM")) > 0
							cMensFis += SF2->F2_XMENCOM
						Else
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							SB1->B1_UM,;
							(cAliasSD2)->D2_QUANT,;
							IIF(!((cAliasSD2)->D2_TIPO$"IP" .Or. ((cAliasSD2)->D2_TIPO $ "D" .And. Len(aNfVinc[nCountIT]) > 9 .And. aNfVinc[nCountIT][10] == "P")) ,IIF(!(lMvNFLeiZF),(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP+(cAliasSD2)->D2_DESCZFC)),IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;
